...
#Область ГлобальныеПеременные
	...
	//Перем гКонфигурацияВерсия; 		// gi_180123 - для учета внутренних различий в конфигурациях, не соответствующих поколениям УТ. //vd_190515
...
Функция ВерсияКонфигурации() Экспорт
	...
	стррРезультат = Новый Структура("Конфигурация,БазоваяКонфигурация"); //vd_190515 стррРезультат = Новый Структура("Конфигурация,БазоваяКонфигурация,КонфигурацияВерсия");
	
	Имя    = Метаданные.Имя;
	Версия = Метаданные.Версия;
	
	стррРезультат.БазоваяКонфигурация = ВерсияКонфигурацииБазоваяКонфигурация(Имя, Версия); //vd_190515
	
	Если    Имя = "УправлениеТорговлей" 
		Или Имя = "УправлениеТорговлейБазовая" 
		Или Имя = "АгентПлюсУправлениеТорговлей" 
	Тогда
		//стррРезультат.БазоваяКонфигурация = "УТ_" + Версия; //vd 191514
	...
	ИначеЕсли Имя = "УправлениеТорговлей_CRM_2" Тогда
		...
		//стррРезультат.КонфигурацияВерсия  = Версия; //vd_190515 
		//}}vd_181009		
		
	ИначеЕсли Имя = "УправлениеТорговлейДляУкраины" Тогда
		//стррРезультат.БазоваяКонфигурация = "УТ_11.1.10"; //vd_190514
		стррРезультат.Конфигурация 	      = "УТ_UA";
		//стррРезультат.КонфигурацияВерсия  = Версия; //vd_180619 //vd_190515
		
	ИначеЕсли Имя = "УправлениеТорговлейДляКазахстана" Тогда
		//((vd_190514
		//Было:
		////vd_180718 Добавлено условие. 
		////Было:
	...
	ИначеЕсли Имя = "КомплекснаяАвтоматизация" Или Имя = "КомплекснаяАвтоматизацияДляКазахстана" Тогда
		...
		//стррРезультат.КонфигурацияВерсия  = Версия; //vd_180912 Было стррРезультат.КонфигурацияВерсия  = "КА_OLD"; //vd_190515
		
	ИначеЕсли Имя = "УправлениеПредприятием" Тогда // ERP 2.X
		//((vd_190514
		//Если СравнитьВерсии(Версия, "2.4") < 0 Тогда
		//	//стррРезультат.БазоваяКонфигурация = "УТ_11.2.3"; //vd_190514
	...
	ИначеЕсли Имя = "KT2000_Alcohol_Trade" Тогда 
		//стррРезультат.БазоваяКонфигурация = "УТ_" + Версия; //vd_190514
		стррРезультат.Конфигурация  = "УТ_АП";
		
	Иначе
		стррРезультат.Конфигурация  			= "УТ";             // vd_181009  Оприделяется поумолчанию  
		//стррРезультат.БазоваяКонфигурация 	= "УТ_11.2"; 	 	// пока неизвестные конфигурации приравниваем к УТ 11.2 //vd_190514 Оприделяется в ВерсияКонфигурацииБазоваяКонфигурация()
		//стррРезультат.КонфигурацияВерсия  	= Неопределено; 	// vd_181009  Оприделяется поумолчанию	//vd_190515
	КонецЕсли;
...
Функция ВерсияКонфигурацииБазоваяКонфигурация(Имя, Версия)
	...
	Если Метаданные.РегистрыСведений.Найти("ВерсииПодсистем") <> Неопределено Тогда
		...
		стррВыборка = РегистрыСведений.ВерсииПодсистем.Получить(Отбор);
		Если ЗначениеЗаполнено(стррВыборка.Версия) Тогда
			БазоваяКонфигурация = "УТ_" + стррВыборка.Версия;
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли Имя = "УправлениеТорговлейДляКазахстана" Тогда
		...
		Если СравнитьВерсии(Версия, "3.4.4.34") = 0 Тогда
			БазоваяКонфигурация = "11.4.4.136";
		ИначеЕсли СравнитьВерсии(Версия, "3.4") >= 0 Тогда
			БазоваяКонфигурация = "11.4";
...
Процедура ОпределитьВерсиюКонфигурации()
	...
	//гКонфигурацияВерсия	 = стррВерсияКонфигурации.КонфигурацияВерсия; //vd_190515
...
Функция ИнициализироватьДанныеВХранилищеДляНовойВерсии() Экспорт
	...
	Если ЭтоПервыйЗапуск Тогда // первый запуск
		...
		стррЗначения.Вставить("РежимВыгрузкиКодовНоменклатуры", "КОД");                                                 //vd_190613
		стррЗначения.Вставить("ВыгружатьТоварыСНулевымОстатком", Истина);                                               //vd_190614
		стррЗначения.Вставить("ЗагружатьДокументыТекущейДатой", Ложь);                                               	//vd_190614
...
Процедура ВыгрузитьНоменклатуру(ДокОбмена, МенеджерВТ)
	...
	Если ИспользоватьПланыПродажПоТоварнымКатегориям Тогда
		стррПоля.Вставить("GrpId2", "");
	КонецЕсли;
	// code //vd_190613 Добавлена выгрузка артикла
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Если ВключатьКодВНаименование Тогда
			стррПоля.Name = ВыборкаНоменклатура.Наименование + " " + СокрЛ(?(флВыгрузкаКода, ВыборкаНоменклатура.Код, ВыборкаНоменклатура.Артикул));  //vd_190613 стррПоля.Name = ВыборкаНоменклатура.Наименование + " " + СокрЛ(ВыборкаНоменклатура.Код);
		Иначе
			стррПоля.Name = ВыборкаНоменклатура.Наименование;
		КонецЕсли; 
		
		//vd_190613 Добавлена выгрузка артикла
...
Процедура ЗаполнитьВыполнениеПланаПродаж(МенеджерВТ, тзПланПродаж)
	...
	Для Каждого стзПериоды Из тзПериоды Цикл
		...
		Для Каждого стзПланПродаж Из мстзПланПродаж Цикл
			...
			ИначеЕсли Не флПланПродажПоНоменклатуре И мОтбора.Найти(стзПланПродаж.ТоварныеКатегории) = Неопределено Тогда // ИначеЕсли мОтбора.Найти(стзПланПродаж.ТоварныеКатегории) = Неопределено Тогда sk_190507 Изменено условие в рамках (MOD-538)
...
Процедура ВыгрузитьДокументыДолг(ДокОбмена, МенеджерВТ)
	...
	мИдЭтапыЗаглушка = Новый Массив; //vd_190517 Заглушка для выгрузки дубликатов документов долг (этапы оплаты дублируют записи, нереализована корректная выгрузка).
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВыгружатьДолгиТолькоПоДокументамАгента Тогда
			Если НЕ(ЗначениеЗаполнено(Выборка.ОбъектРасчетов) И ПроверитьНаличиеРеквизита(Выборка.ОбъектРасчетов, "Менеджер") И Выборка.ОбъектРасчетов.Менеджер = ВыбАгент) Тогда
				Если НЕ(ЗначениеЗаполнено(Выборка.РасчетныйДокумент) И ПроверитьНаличиеРеквизита(Выборка.РасчетныйДокумент, "Менеджер") И Выборка.РасчетныйДокумент.Менеджер = ВыбАгент) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;	
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДокументОснование) И ТипЗнч(ДокументОснование) = ТипДокументЗаказ ИЛИ ТипЗнч(ДокументОснование) = ТипДокументРеализация Тогда
			...
			//((vd_190517 Заглушка для выгрузки дубликатов документов долг (этапы оплаты дублируют записи).
...
Функция ВидыЦенВыгрузки(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |
				   //vd_190613 АВТОУПОРЯДОЧИВАНИЕ некорректно сортирует выгрузку номенклатуры |УПОРЯДОЧИТЬ ПО Cсылка АВТОУПОРЯДОЧИВАНИЕ";
				   |УПОРЯДОЧИТЬ ПО
...
Процедура ЗаполнитьРежимыВыгрузкиКодовНоменклатуры(СписокВыбора) Экспорт
	...
	СписокВыбора.Очистить();
	//СписокВыбора.Добавить("", 		 НСтр("ru = 'Не выгружать'")); //vd_190613
...
Функция ДанныеПоНоменклатуре(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |
   				   //vd_190613 АВТОУПОРЯДОЧИВАНИЕ некорректно сортирует выгрузку номенклатуры |УПОРЯДОЧИТЬ ПО Номенклатура АВТОУПОРЯДОЧИВАНИЕ";
	               |УПОРЯДОЧИТЬ ПО
...
Функция ДанныеПоХарактеристикамНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |
   				   //vd_190613 АВТОУПОРЯДОЧИВАНИЕ некорректно сортирует выгрузку номенклатуры |УПОРЯДОЧИТЬ ПО Характеристика АВТОУПОРЯДОЧИВАНИЕ";
				   |УПОРЯДОЧИТЬ ПО
...
Функция ДанныеПоОстаткамНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |;
	               |
   				   //((vd_190613 АВТОУПОРЯДОЧИВАНИЕ некорректно сортирует выгрузку номенклатуры 
                   //Было:				   
				   //|ВЫБРАТЬ
...
Функция ДанныеПоОстаткамХарактеристикНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_ХарактеристикаСклады.Склад КАК Склад,
	               |	СУММА(ЕСТЬNULL(ВТ_НоменклатураОстатки.Остаток, 0)) КАК Остаток
				   |ПОМЕСТИТЬ ВТ_ХарактеристикаСортировка
	               |ИЗ
	               |	ВТ_ХарактеристикаСклады КАК ВТ_ХарактеристикаСклады
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_НоменклатураОстатки КАК ВТ_НоменклатураОстатки
	               |		ПО ВТ_ХарактеристикаСклады.Характеристика = ВТ_НоменклатураОстатки.Характеристика
				   ...
				   |////////////////////////////////////////////////////////////////////////////////
				   |ВЫБРАТЬ
				   |	ВТ_ХарактеристикаСортировка.Характеристика КАК Характеристика,
				   |	ВТ_ХарактеристикаСортировка.Склад КАК Склад,
				   |	ВТ_ХарактеристикаСортировка.Остаток КАК Остаток
				   |ИЗ
				   |	ВТ_ХарактеристикаСортировка КАК ВТ_ХарактеристикаСортировка
				   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ВТ_СпрХарактеристикиНоменклатуры
	               |		ПО ВТ_ХарактеристикаСортировка.Характеристика = ВТ_СпрХарактеристикиНоменклатуры.Ссылка
				   |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК ВТ_СпрСклады
	               |		ПО ВТ_ХарактеристикаСортировка.Склад = ВТ_СпрСклады.Ссылка
				   |
				   |УПОРЯДОЧИТЬ ПО
	               ...
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ ВТ_ХарактеристикаСортировка";
...
Функция ДанныеПоЦенамНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //))vd_190613
	               |
				   //vd_190613 АВТОУПОРЯДОЧИВАНИЕ некорректно сортирует выгрузку номенклатуры |УПОРЯДОЧИТЬ ПО ВТ_НоменклатураВидЦены.Номенклатура, ВТ_НоменклатураВидЦены.ВидЦены
				   |УПОРЯДОЧИТЬ ПО
	               |	СпрНоменклатура.Наименование,
				   |	СпрНоменклатура.Код,
				   |	СпрНоменклатура.Артикул,
...
Функция ДанныеПоЦенамХарактеристикНоменклатуры(МенеджерВТ)
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   //))vd_190613
	               |
				   //vd_190613 АВТОУПОРЯДОЧИВАНИЕ некорректно сортирует выгрузку номенклатуры |УПОРЯДОЧИТЬ ПО СпрХарактеристикиНоменклатуры.Наименование, СпрВидыЦен.Наименование
				   |УПОРЯДОЧИТЬ ПО
	               |	СпрХарактеристикиНоменклатуры.Наименование,
	               |	СпрВидыЦен.Наименование
	               |ИТОГИ
...
Функция ДанныеПоЕдиницамИзмеренияВыгрузки(МенеджерВТ)
	...
	Если КонстантыПолучитьЗначение("ИспользоватьУпаковкиНоменклатуры") Тогда
		Запрос.Текст = 
			...
			|	"""" КАК КодОКЕИ,
			//vd_190614 не выгружать помеченные на удаление еденицы измерения	Номенклатура.ЕдиницаИзмерения.Наименование КАК Наименование,
			...
			//vd_190614 не выгружать помеченные на удаление еденицы измерения
			...
			|	Номенклатура.ИспользоватьУпаковки
			|	И Номенклатура.НаборУпаковок <> &ИндивидуальныйНаборУпаковок
			//((vd_190614 не выгружать помеченные на удаление еденицы измерения. Оптимизация - добавлена индексация по полю Классификатор
			|	И Упаковки.ПометкаУдаления = Ложь
	...
	Иначе // не используются упаковки номенклатуры
		...
		Запрос.Текст = 
			...
			//((vd_190614 не выгружать помеченные на удаление еденицы измерения. Оптимизация - Индексация
	...
	Запрос.Текст = Запрос.Текст + "
	    //((vd_190613 не выгружать помеченные на удаление еденицы измерения. Оптимизация, создана дополнительная ВТ.
		|;
		|
		|////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ДанныеПоДолгам(МенеджерВТ)	
	...
	//vd_190522 Добавлена индексация по полю КлючАналитики
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Иначе
		...
		Если ПоколениеКонфигурации(">=УТ_11.2") Тогда
			...
			//vd_190522 Добавлена индексация
			Запрос.Текст ="ВЫБРАТЬ
			...
			Запрос.Текст ="ВЫБРАТЬ
			              ...
			              |	РасчетыСКлиентамиПоДокументамОстатки.ДолгОстаток КАК ДолгОстатокТекущий,
			              |	РасчетыСКлиентамиПоДокументамОстатки.ДолгРеглОстаток КАК ДолгРегОстатокТекущий
			...
			Запрос.Текст ="ВЫБРАТЬ
			              ...
			              |	ВТ_ДолгОстатки.ДолгОстатокТекущий КАК ДолгОстаток,
			              |	ВТ_ДолгОстатки.ДолгРегОстатокТекущий КАК ДолгРегОстаток,
			              |	ВТ_ДолгНачальный.ДолгНачальный КАК ДолгНачальныйЭтапа,
			              |	ВТ_ДолгНачальный.ДолгНачальный * ВТ_КурсВалютыУчета.КоэффициентПересчета КАК ДолгНачальныйРегЭтапа,
						  |	ВТ_ДолгОстатки.ДолгРегОстатокТекущий КАК КОплатеОстаток
			              |ПОМЕСТИТЬ ВТ_Долги
			              |ИЗ
			              ...
			              |		ПО ВТ_ДолгОстатки.Валюта = ВТ_КурсВалютыУчета.Валюта";
		...
		Иначе //ПоколениеКонфигурации("<УТ_11.2") Тогда
			...
			Запрос.Текст ="ВЫБРАТЬ
			              ...
			              |	РасчетыСКлиентамиПоДокументам.ДолгОстаток КАК ДолгОстаток,
						  |	РасчетыСКлиентамиПоДокументам.ДолгРеглОстаток КАК ДолгРеглОстаток
						  |	РасчетыСКлиентамиПоДокументам.ДолгРеглОстаток КАК КОплатеОстаток
			              |ПОМЕСТИТЬ ВТ_Долги
			              |ИЗ
			              |		РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки(&ДатаОстатков, ) КАК РасчетыСКлиентамиПоДокументам
			              |ГДЕ
			              | РасчетыСКлиентамиПоДокументам.ДолгРеглОстаток > 0";
...
Функция ТекстЗапросаПолученияИзображений()
	...
	//vd_190515 Добавлено корректное оприделение базовой версии через ВерсииПодсистем
...
Процедура МенеджерВТНоменклатураОстатки(МенеджерВТ)
    ...
    //(( sk_190429 При расчете остатка с учетом резерва учитываем сумму резерва под заказ в рамках (MOD-517)	
	...
	Если РегистрОстатковТоваров = "СвободныеОстатки" Тогда
		...
		Если ПоколениеКонфигурации("<=УТ_11.1.2") Тогда
			стррЛитералы.ВРезервеСоСкладаОстаток = "ВРезервеОстаток";
			стррЛитералы.ВРезервеПодЗаказОстаток = "0"; // sk_190429 Добавлено в рамках (MOD-517) 
		Иначе
			стррЛитералы.ВРезервеСоСкладаОстаток = "ВРезервеСоСкладаОстаток";
			стррЛитералы.ВРезервеПодЗаказОстаток = "СвободныеОстаткиТоваров.ВРезервеПодЗаказОстаток"; // sk_190429 Добавлено в рамках (MOD-517) 
		КонецЕсли;
	Иначе 
		стррЛитералы.РегистрНакопленияОстатков 	= "ТоварыНаСкладах";
		стррЛитералы.ВРезервеСоСкладаОстаток 	= "КОтгрузкеОстаток";
	...
	Запрос = Новый Запрос;
	//(( sk_190429 Запрос изменен в рамках (MOD-517) 
	//Запрос.Текст = "ВЫБРАТЬ
	//               |	ВТ_Ассортимент.Ссылка КАК Номенклатура,
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |СГРУППИРОВАТЬ ПО
	               |	ВТ_Ассортимент.Ссылка,
				   |	ЕСТЬNULL(СвободныеОстаткиТоваров.Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.Пустаяссылка)),
   				   |	ЕСТЬNULL(СвободныеОстаткиТоваров.Склад, ЗНАЧЕНИЕ(Справочник.Склады.Пустаяссылка))
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Номенклатура,
	               |	Характеристика";
...
Функция ЗагрузитьДокументЗаказ(ТекстXML, стррПараметры, ЗагружаемыеОбъекты)
	...
	//{{vd_190614
	//Зполняет реквизиты объекта по соглашению, изменяет склад
...
Функция ЗагрузитьДокументРеализация(ТекстXML, стррПараметры, ЗагружаемыеОбъекты)
	...
	Если ИспользоватьПунктыРазгрузки И НЕ РежимВыгрузкиРекомендуемый Тогда
		...
		ИначеЕсли ИспользоватьКонтрагентов Тогда
			стррОбязательныеПоля.Вставить("Договор");
		ИначеЕсли КонстантыПолучитьЗначение("ИспользованиеСоглашенийСКлиентами") <> Перечисления.ИспользованиеСоглашенийСКлиентами.НеИспользовать Тогда // sk_190517 Проверяем параметр использования соглашений с клиентами в учетной системе в рамках (MOD-553)
			стррОбязательныеПоля.Вставить("Соглашение");
		КонецЕсли;
		//}}vd_180503 
	...
	Иначе
		...
		КонецЕсли;
		//}}vd_180413 Баг 38286. ЗагрузкаКонтрагентов.
		//(( sk_190517 Проверяем параметр использования соглашений с клиентами в учетной системе в рамках (MOD-553) 
		//стррОбязательныеПоля = Новый Структура("Организация, Партнер, Контрагент, Соглашение");
	...
	//vd_180716 Заполняет и перезаполняет реквизиты "Руководитель" и "Главный бухгалтер"
	ОтветственныеЛицаСервер.ПриИзмененииСвязанныхРеквизитовДокумента(ОбъектДок);  
	...
	Если ЗначениеЗаполнено(Соглашение) Тогда	
		...
		//ОбъектДок.Валюта = ВалютаУчета; //vd_190614
	...
	Иначе 
		...
		//(( sk_190517 Используем процедуру ДокументПродажиЗаполнить() в рамках (MOD-553)  
		//ОрганизацияДоИзменения = ОбъектДок.Организация;
		...
		//ЗаполнитьНалогооблажениеНДСДокумента(ОбъектДок); // sk_190517 Заполнение НДС выполняется в процедуре ДокументПродажиЗаполнить() 
		//}}vd_180912 создана процедура для заполнения реквизита НалогообложениеНДС
...
Процедура ЗаполнитьОсновныеРеквизитыДокумента(ОбъектДок, стррЗначения, ДокументМенеджер = Неопределено, стррОбязательныеПоля)
	...
	//((vd_190614 Если документ уже записан ы ЦБД, то дата не перезаписывается.
	//Было:
	//ОбъектДок.Дата = ДатаДок;
	Если ЗагружатьДокументыТекущейДатой И ОбъектДок.Дата = '00010101' Тогда
...
Процедура ЗаполнитьНалогооблажениеНДСДокумента(ОбъектДок)
	...
	//УТ_KZ 3.4.4.34 	- нет	- 11.4.4.136 	//vd_190515
	//УТ 11.4.5.24      - есть  - 11.4.5.24     
	//КА 2.4.3.137      - есть  - 
	//КА 2.4.5.33       - нет   - 
	//Если ПоколениеКонфигурации("<УТ_11.4.5.24") И ЗначениеЗаполнено(ОбъектДок.Соглашение) Тогда //vd_190515
    ...
    Если НалогообложениеНДС = неопределено Тогда 
		//УТ_KZ 3.4.4.34 	- НалогообложениеНДС  - 11.4.4.136 	//vd_190515
		//УТ 11.4.5.24      - НалогообложениеНДС  - 11.4.5.24  
		//vd_190515 Добавлено корректное оприделение базовой версии через ВерсииПодсистем
		//Если ПоколениеКонфигурации(">=УТ_11.4.5.24") ИЛИ (гКонфигурация = "КА" И СравнитьВерсии(гКонфигурацияВерсия, "2.4.5") >= 0) Тогда ... и изменить местами условие        
...
Процедура ДокументПродажиЗаполнить(ОбъектДок)
	...
	//(( sk_190517 В рамках (MOD-553)
	Если Не ЗначениеЗаполнено(ОбъектДок.Валюта) Тогда
		ОбъектДок.Валюта = КонстантыПолучитьЗначение("ВалютаРегламентированногоУчета");	
	КонецЕсли;
	//)) sk_190517	
	
	//(( sk_190517 Для заполнения НДС используем процедуру ЗаполнитьНалогооблажениеНДСДокумента() в рамках (MOD-553)
...
Процедура ЗаполнитьУсловияПродажПоСоглашению(ОбъектДок)
	...
	//vd_190513 Использование гКонфигурацияВерсия убрано._	
