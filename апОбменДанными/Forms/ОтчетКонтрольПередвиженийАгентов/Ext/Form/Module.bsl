...
Функция КартаОбновитьТекущийМакет()
	...
	////vd_190617 Добавлено использование WebKit
		...
		Для каждого СтрокаТ Из тзПланФактПосешений Цикл
			...
			Если ЕстьФакт Тогда // добавляем в дерево документы
				...
				Для каждого стзФакт Из мСтроки Цикл
					...
					////))vd_190128
					Информация = "";
					//Время начала или время окончания из МУ отличается от даты отчета.
					Если НачалоДня(стзФакт.ВремяНачала) = НачалоДня(ДатаОтчета) И НачалоДня(стзФакт.ВремяОкончания) = НачалоДня(ДатаОтчета) Тогда 
						ДатаНач = стзФакт.ВремяНачала;
						ДатаКон = стзФакт.ВремяОкончания;
					Иначе
						ДатаНач = ?(стзФакт.ВремяНачала >= ДатаОтчета, стзФакт.ВремяНачала, ДатаОтчета);
						ДатаКон = ?(стзФакт.ВремяОкончания <= КонецДня(ДатаОтчета), стзФакт.ВремяОкончания, КонецДня(ДатаОтчета));
						Информация = "Дата создания в МУ: " + Формат(стзФакт.ВремяНачала, "ДФ=""dd.MM.yyyy HH:mm""") + " - " + Формат(стзФакт.ВремяОкончания, "ДФ=""dd.MM.yyyy HH:mm""") + " ";
					КонецЕсли;
					//Дата документа отличается от даты создания в МУ.
					Если НЕ ЗначениеЗаполнено(стзФакт.Дата) ИЛИ НЕ НачалоДня(стзФакт.ВремяНачала) = НачалоДня(стзФакт.Дата) ИЛИ НЕ НачалоДня(стзФакт.ВремяОкончания) = НачалоДня(стзФакт.Дата) Тогда
						Если НЕ ЗначениеЗаполнено(стзФакт.Дата) Тогда
							Информация = "Нет даты документа."; 
						иначе
							Информация = "Дата документа: " + Формат(стзФакт.Дата, "ДФ=""dd.MM.yyyy HH:mm"". ");
							Информация = Информация + "Дата создания в МУ: " + Формат(стзФакт.ВремяНачала, "ДФ=""dd.MM.yyyy HH:mm""") + " - " + Формат(стзФакт.ВремяОкончания, "ДФ=""dd.MM.yyyy HH:mm""") + " ";
						КонецЕсли;	
					КонецЕсли;
				    //Строка времени в дереве ПланФакт на форме
					сдзПланФактУровеньДокумент.Время = Формат(ДатаНач, "ДФ=""HH:mm""") + ЗнакВнеТТ + " - " + Формат(ДатаКон, "ДФ=""HH:mm""") + ЗнакВнеТТ;
					//Если есть замечания то в информация добавляется "(i)".
